...
2. 要使用 **Hadoop** 命令运行示例 MapReduce 作业，请使用以下命令：
	...
	这将启动 **wordcount** 类（包含在当前目录下的 **hadoop-mapreduce-examples.jar** 文件中）。它使用 ****wasb://example/data/gutenberg/davinci.txt** 文档作为输入，输出将存储到 ****wasb:///example/data/WordCountOutput**。
...
3. 作业完成时，使用以下命令列出存储在 ****wasb://example/data/WordCountOutput** 上的输出文件：

<properties
	...
	description="在 HDInsight 中的 Hadoop 群集上运行 MapReduce 单词计数示例。该程序用 Java 编写，对单词出现在文本文件中的次数进行统计。"
	editor="cgronlun"
	manager="paulettm"
	services="hdinsight"
	documentationCenter=""
...
了解如何使用 Azure PowerShell 在 HDInsight 中的 Hadoop 群集上运行 MapReduce 程序。该程序用 Java 编写，它对单词出现在文本文件中的次数进行统计，然后输出包含每个单词与其出现频率配对的新文本文件。

将该程序安装到群集上。本教程中分析的文本文件是《莱奥纳多·达·芬奇笔记》(The Notebooks of Leonardo Da Vinci) 的 Project Gutenberg 电子书版本。
...
- **HDInsight 群集**。有关可用于创建此类群集的各种不同方法的说明，请参阅 [Azure HDInsight 入门][hdinsight-get-started]或[预配 HDInsight 群集](/documentation/articles/hdinsight-provision-clusters)。
...
WordCount 脚本的输出应显示在命令窗口中：

![在 HDInsight 中运行 Hadoop MapReduce 单词计数示例后，在 PowerShell 中生成单词频率结果。][image-hdi-sample-wordcount-output]

请注意，MapReduce 作业的输出文件是不可变的。因此，如果重新运行此示例，需要更改输出文件的名称。

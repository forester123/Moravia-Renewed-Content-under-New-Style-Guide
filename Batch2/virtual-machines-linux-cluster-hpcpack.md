...
* **内核配额** - 你可能需要增加内核配额，尤其是在选择部署具有多核 VM 大小的多个群集节点时。对于本文中的示例，你将至少需要 12 个可用内核。若要增加配额，可免费[建立联机客户支持请求](http://azure.microsoft.com/blog/2014/06/04/azure-limits-quotas-increase-requests/)。
...
* **VNet** - 将在其中创建 HPC 群集的虚拟网络和子网的设置。在运行此脚本之前，你可以自己创建虚拟网络和子网，否则此脚本将使用地址空间 192.168.0.0/20 创建虚拟网络，使用地址空间 192.168.0.0/23 创建子网。在本示例中，此脚本将创建虚拟网络 centos7rdmavnetje 和子网 CentOS7RDMACluster。
...
* 支持对 A8 和 A9 大小 VM 建立 RDMA 连接的 Linux 映像已可用。如果你指定的映像安装并启用了 Linux RDMA 驱动程序，则 HPC Pack IaaS 部署脚本将部署这些驱动程序。例如，为当前的 SUSE Linux Enterprise Server 12（已针对应用商店中的高性能计算映像进行优化）指定映像名称 `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-v20150708`。
...
你可以使用多种方式在群集的 Linux 节点和 Windows 头节点之间移动数据。以下是三种常用方法。
...
1. 在头节点上创建一个文件夹并与具有读/写权限的所有用户共享。例如，在头节点上将 D:\\OpenFOAM 共享为 \\CentOS7RDMA-HN\\OpenFOAM。在此处，CentOS7RDMA-HN 是头节点的主机名。
...
* 尝试 [Azure 快速入门模板](https://azure.microsoft.com/documentation/templates/create-hpc-cluster-linux-cn/)，组合 Azure Resource Manager 来加快部署 HPC Pack 与大量 Linux 计算节点。

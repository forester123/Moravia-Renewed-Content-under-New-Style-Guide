...
# 使用 Azure Resource Manager 模板和 Azure CLI 部署和管理虚拟机
...
本文说明如何使用 Azure Resource Manager 模板和 Azure CLI 来执行以下常见任务，以便部署和管理 Azure 虚拟机。有关可用的其他模板，请参阅 [Azure 快速入门模板](https://azure.microsoft.com/documentation/templates/)和[使用模板的应用程序框架](/documentation/articles/virtual-machines-app-frameworks)。

[AZURE.INCLUDE [learn-about-deployment-models](../includes/learn-about-deployment-models-rm-include.md)]经典部署模型。你不能在经典部署模型中使用模板。
...
必须拥有正确的 Azure CLI 版本和 Azure 帐户，才能将 Azure CLI 与 Azure 资源组配合使用。如果没有 Azure CLI，请[安装](/documentation/articles/xplat-cli-install)。

### 将 Azure CLI 版本更新到 0.9.0 或更高版本
...
你也可以使用以下 [Docker 映像](https://registry.hub.docker.com/u/microsoft/azure-cli/)将 Azure CLI作为 Docker 容器运行。在 Docker 主机上运行以下命令：
...
大多数应用程序是通过不同资源类型的组合（例如，一个或多个 VM 和存储帐户、一个 SQL 数据库、一个虚拟网络或内容交付网络）构建的。默认的 Azure 服务管理 API 和 Azure 经典门户使用基于服务的方法代表这些项。这种方法需要你单独部署和管理各个服务（或查找其他具备相同功能的工具），而不是当作单个逻辑部署单元。
...
然后，使用 Azure CLI 资源管理命令执行以下操作，即可管理组资源的整体生命周期：
...
可在 [Azure Resource Manager 概述](/documentation/articles/resource-group-overview)中了解有关 Azure 资源组及其功能的详细信息。如果你想要了解如何创作模板，请参阅[创作 Azure Resource Manager 模板](/documentation/articles/resource-group-authoring-templates)。
...
请参考以下各部分中的说明，使用 Azure CLI 从模板部署新的 Azure VM。此模板会在只有单个子网的新虚拟网络中创建单个虚拟机，而不同于 `azure vm quick-create`，它可以让你确切描述想要的内容，而且重复使用时也不会发生任何错误。以下是此模板创建的内容：
...
模板很灵活，因此，设计人员可能已经选择提供很多的参数给你，或者选择创建一个更固定的模板，而只提供几个参数给你。为了收集信息，请将这个模板以参数的形式传递，然后打开模板文件（本主题内嵌了以下模板），然后检查**参数**值。
...
- 模板文件（如果将上述 JSON 模板保存到本地文件）。
- 模板 URI（如果要指向 Github 中的文件或其他网址）。
- 要部署到的资源组。
- 可选的部署名称。

系统会提示你提供 JSON 文件“参数”部分中的参数的值。指定所有参数值后，即会开始部署。
...
你已了解上述模板的基本用法，那么现在我们可以使用类似的说明，通过 Azure CLI 使用模板从 Azure 中的特定 .vhd 文件创建自定义 VM 映像。其中的差别就是此模板会从指定的虚拟硬盘 (VHD) 创建单个虚拟机。
...
对于基于 Linux 的虚拟机，请参阅[创建和上载包含 Linux 操作系统的虚拟硬盘](/documentation/articles/virtual-machines-linux-create-upload-vhd)。
...
然后使用 `--template-uri` 选项直接调用模板（或者通过 `--template-file` 选项，使用自己保存在本地的文件），开始创建部署。请注意，因为模板已指定默认值，所以系统会提示你只输入几项数据。如果将模板部署到几个不同的地方，你可能会发现某些名称与默认值冲突（特别是你创建的 DNS 名称）。
...
以下是模板的 JSON 文件内容。如果你需要最新版本，可查阅 [Github 模板存储库](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/201-2-vms-loadbalancer-lbrules/azuredeploy.json)。此主题使用 `--template-uri` 开关来调用模板，不过你也可以使用 `--template-file` 开关来传递本地版本。
...
使用 `azure group create <location>` 为模板创建资源组。然后在该资源组中创建部署，方法是使用 `azure group deployment create` 并传递资源组、部署名称，然后根据提示为模板中没有默认值的参数输入相关的值。
...
你可以快速发现问题所在，进行修复，然后再试一次。在以下情况下，模板同时创建了两个 VM，导致在 .vhd 中创建了锁。（修改模板后，很快就部署成功。）
...
有关 Azure CLI 用法和 **arm** 模式的更多示例，请参阅[将适用于 Mac、Linux 和 Windows 的 Azure CLI 用于 Azure Resource Manager](/documentation/articles/xplat-cli-azure-resource-manager)。若要了解有关 Azure 资源及其概念的详细信息，请参阅 [Azure Resource Manager 概述](/documentation/articles/resource-group-overview)。

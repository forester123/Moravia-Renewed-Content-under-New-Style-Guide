...
> [AZURE.NOTE] Azure 有两个不同的用于创建和处理资源的部署模型：[资源管理器和经典](/documentation/articles/resource-manager-deployment-model)。本文介绍如何使用资源管理器部署模型。Microsoft 建议对大多数新的部署使用该模型，而不是经典部署模型。

...
- **已安装和配置 Azure PowerShell 的工作站**。有关说明，请参阅 [安装和配置 Azure PowerShell][powershell-install]。要执行 PowerShell 脚本，必须以管理员身份运行 Azure PowerShell 并将执行策略设为 *RemoteSigned*。请参阅 [使用 Set-ExecutionPolicy cmdlet][2]。
...
Azure 门户不支持使用自定义配置选项设置 HDInsight 群集。例如，将 *hbase.replication* 设置为 *true*。如果设置群集后在配置文件中设置该值，重新创建群集映像后会丢失该设置。有关详细信息，请参阅[在 HDInsight 中预配 Hadoop 群集][hdinsight-provision]。使用自定义选项设置 HDInsight 群集的选项之一是使用 Azure PowerShell。
...
2.	单击“管理工具”。
3.	单击“DNS”。
4.	在左窗格中，依次展开“DSN”、“Contoso-DNS-EU”。
5.	右键单击“条件转发器”，然后单击“新建条件转发器”。 
5.	输入以下信息：
	- **DNS 域**：输入 Contoso-HBase-US 的 DNS 后缀。示例：Contoso-HBase-US.f5.internal.chinacloudapp.cn.
	- **主服务器的 IP 地址**：输入 10.2.0.4，这是 Contoso-DNS-US 的 IP 地址。请验证该 IP。DNS 服务器可以有不同的 IP 地址。
6.	按 **ENTER**，然后单击“确定”。现在，可以从 Contoso-DNS-EU 解析 Contoso-DNS-US 的 IP 地址。
...
- 重复上述步骤，在 Contoso-HBase-US 中创建相同的表。
...
2. 在 HBase shell 窗口中，添加目标群集 (Contoso-HBase-US) 作为对等方，例如：
	...
	在示例中，域后缀是 *contoso-hbase us.d4.internal.chinacloudapp.cn*。将其更新为匹配美国 HBase 群集的域后缀。主机名之间不能有空格。

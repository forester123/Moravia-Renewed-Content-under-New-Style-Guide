...
在本部分中，使用 **<a href = "http://msdn.microsoft.com/zh-cn/library/dn858088.aspx" target="_blank">Add-AzureHDInsightScriptAction</a>** cmdlet 通过脚本操作来调用脚本，以自定义群集。在继续前，确保已安装和配置 Azure PowerShell。有关配置工作站以运行 HDInsight 的 Azure PowerShell cmdlet 的信息，请参阅 [安装并配置 Azure PowerShell][powershell-install-configure]。

...
本文提供了用于使用[适用于 .NET 的 Azure SQL 数据库库](https://www.nuget.org/packages/Microsoft.Azure.Management.Sql)通过 C# 执行许多 Azure SQL 数据库管理任务的命令。

为简明起见，我们已分开列出各个代码片段，并在本文底部的某个部分中提供了一个示例控制台应用程序，其中结合了所有命令。
...
如果你没有 Azure 订阅，只需单击本页顶部的“试用”，然后再返回到本文中。如需 Visual Studio 的免费副本，请参阅 [Visual Studio 下载](https://www.visualstudio.com/downloads/download-visual-studio-vs)页。

## 安装所需的库

使用[程序包管理器控制台](http://docs.nuget.org/Consume/Package-Manager-Console)安装以下程序包，即可获取所需的管理库：
...
必须先通过设置所需的身份验证，使应用程序能够访问 REST API。

[Azure Resource Manager REST API](https://msdn.microsoft.com/zh-cn/library/azure/dn948464.aspx) 使用 Azure Active Directory 进行身份验证，而不是早期 Azure 服务管理 REST API 使用的证书。

若要基于当前的用户对客户端应用程序进行身份验证，你必须先将该应用程序注册到与创建了 Azure 资源的订阅关联的 AAD 域中。如果 Azure 订阅已通过 Microsoft 帐户而不是工作或学校帐户创建，则你已经有了默认的 AAD 域。可以在[管理门户](https://manage.windowsazure.cn)中完成应用程序的注册。
...
7. 完成创建应用、单击“配置”，然后复制“客户端 ID”（需要在代码中使用客户端 ID）。
...
*创建一个池后，你还可以使用 Transact-SQL 在池中新建弹性数据库。有关详细信息，请参阅[弹性数据库池参考 - Transact-SQL](/documentation/articles/sql-database-elastic-pool-reference/#Transact-SQL)。*

若要直接在池中新建数据库，请执行以下操作：
...
若要删除某个服务器（这也会删除该服务器上的数据库和所有弹性数据库池），请运行以下代码：

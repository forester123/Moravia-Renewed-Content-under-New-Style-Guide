...
此外，[Azure 门户](https://portal.azure.cn)还可以提供影响各种 Azure 服务的事件的通知。注意：此信息以前已在 [Azure 服务仪表板](/support/service-dashboard/)上与历史数据一起提供。
...
- [Common REST API Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179357.aspx)（常见的 REST API 错误代码）
- [Blob Service Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179439.aspx)（Blob 服务错误代码）
- [Queue Service Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179446.aspx)（队列服务错误代码）
- [Table Service Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179438.aspx)（表服务错误代码）
- [File Service Error Codes](https://msdn.microsoft.com/zh-cn/library/azure/dn690119.aspx)（文件服务错误代码）
...
你应查看客户端日志以了解客户端应用程序正在提交多少个请求，并检查客户端中与 .NET 相关的常规性能瓶颈（如 CPU、.NET 垃圾回收、网络利用率或内存）。作为排查 .NET 客户端应用程序问题的起点，请参阅 [Debugging, Tracing, and Profiling](http://msdn.microsoft.com/zh-cn/library/7fe0dd2y)（调试、跟踪和分析）。
...
如果您看到 **PercentThrottlingError** 的值达到峰值的时间与应用程序活动的高峰期保持一致，则应在客户端中对重试实施指数（而非线性）退让策略：这将减少分区上的即时负载，并帮助您的应用程序消除流量峰值。有关如何使用存储客户端库实现重试策略的详细信息，请参阅 [Microsoft.WindowsAzure.Storage.RetryPolicies Namespace](http://msdn.microsoft.com/zh-cn/library/azure/microsoft.windowsazure.storage.retrypolicies.aspx)（Microsoft.WindowsAzure.Storage.RetryPolicies 命名空间）。
...
如果你使用存储客户端库生成 SAS 令牌，则可轻松生成有效令牌。但是，如果使用的是存储 REST API 并手动构造 SAS 令牌，则应仔细阅读主题 [Delegating Access with a Shared Access Signature](http://msdn.microsoft.com/zh-cn/library/azure/ee395415.aspx)（使用共享访问签名委托访问）。
...
若要解决此 JavaScript 问题，可以为客户端访问的存储服务配置跨域资源共享 (CORS)。有关详细信息，请参阅 [Cross-Origin Resource Sharing (CORS) Support for Azure Storage Services](http://msdn.microsoft.com/zh-cn/library/azure/dn535601.aspx)（Azure 存储服务的跨域资源共享 (CORS) 支持）。
...
可以在 [Common REST API Error Codes](http://msdn.microsoft.com/zh-cn/library/azure/dd179357.aspx)（常见的 REST API 错误代码）页上找到存储服务返回的常见 REST API 错误代码的列表。
...
> [AZURE.NOTE] 有关使用 Wireshark 的详细信息，请参阅 [Wireshark Users Guide](http://www.wireshark.org/docs/wsug_html_chunked)（Wireshark 用户指南）。
...
有关 Microsoft Message Analyzer **Web 代理**跟踪的详细信息，请参阅 [Microsoft-PEF-WebProxy Provider](http://technet.microsoft.com/zh-cn/library/jj674814.aspx)（Microsoft-PEF-WebProxy 提供程序）。
...
有关 Microsoft Message Analyzer 本地链路层跟踪的详细信息，请参阅 [Microsoft-PEF-NDIS-PacketCapture Provider](http://technet.microsoft.com/zh-cn/library/jj659264.aspx)（Microsoft-PEF-NDIS-PacketCapture 提供程序）。

### <a name="appendix-4"></a>附录 4：使用 Excel 查看度量值和日志数据

使用许多工具可以从 Azure 表存储中下载带分隔符格式的存储度量值数据，以便可以轻松地将这些数据加载到 Excel 中进行查看和分析。来自 Azure Blob 存储的存储日志记录数据已采用可以加载到 Excel 中的带分隔符格式。但是，需要基于 [Storage Analytics Log Format](http://msdn.microsoft.com/zh-cn/library/azure/hh343259.aspx)（存储分析日志格式）和 [Storage Analytics Metrics Table Schema](http://msdn.microsoft.com/zh-cn/library/azure/hh343264.aspx)（存储分析度量值表架构）中的信息添加相应的列标题。
...
可以在[什么是 Application Insights？](/documentation/articles/app-insights-overview/)中找到详细信息。

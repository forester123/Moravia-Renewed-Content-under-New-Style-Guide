...
一旦通过 API 将实时编码通道的状态转换为“正在运行”，就会开始计费。可在 Azure 管理门户或 Azure 媒体服务资源管理器工具 (http://aka.ms/amse) 中查看状态。

下表显示了通道状态如何映射为 API 和 Azure 管理门户中的计费状态。请注意，API 与门户 UX 之间的状态略有不同。一旦通过 API 将通道置于“正在运行”状态，或者在 Azure 管理门户中将其设置为“就绪”或“正在流式处理”状态，就会开始计费。若要阻止通道进一步计费，必须通过 API 或 Azure 管理门户停止通道。使用完实时编码通道后，需要亲自停止通道。不停止编码通道会导致持续计费。
...
>[AZURE.NOTE]当前，不管指定了哪种输入类型，都只能以分片 MP4（平滑流）格式来传送预览流。可以使用 [http://smf.cloudapp.net/healthmonitor](http://smf.cloudapp.net/healthmonitor) 播放器来测试平滑流。还可以使用 Azure 管理门户中承载的播放器来查看流。

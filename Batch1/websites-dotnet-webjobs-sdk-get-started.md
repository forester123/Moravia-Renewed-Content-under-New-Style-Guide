...
## <a name="configure-storage"></a><a name="configure-the-web-app-to-use-your-azure-sql-database-and-storage-account"></a>将应用程序配置为使用存储帐户
...
2. 转到 [Azure 经典管理门户](http://manage.windowsazure.cn/)，然后单击包含数据库的 Web 应用。
...
9. 添加名为 AzureWebJobsStorage 和 AzureWebJobsDashboard 的两个新连接字符串。将类型设置为“自定义”，并将连接字符串值设置为前面针对 *Web.config* 和 *App.config* 文件使用的相同值。（确保包括整个连接字符串而不仅仅是访问密钥，且不包括引号。）
...
12. Web 应用停止后，再次右键单击该 Web 应用，然后单击“启动”。

	发布时 Web 作业会自动启动，但在更改配置时会停止。若要重新启动，可以重新启动 Web 应用或在 [Azure 经典管理门户](https://manage.windowsazure.cn/)中重新启动 Web 作业。通常建议在更改配置后重新启动 Web 应用。
...
>[AZURE.NOTE] 完成测试后，请删除 Web 应用和 SQL 数据库实例。Web 应用免费，但 SQL 数据库实例和存储帐户收费（由于较小，因此费用很低）。此外，如果保持 Web 应用运行，则找到 URL 的任何人都可以创建和查看广告。在经典管理门户中，转到 Web 应用的“仪表板”选项卡，然后单击页面底部的“删除”按钮。然后，可以选中用于同时删除 SQL 数据库实例的复选框。如果只想暂时防止其他人访问 Web 应用，请单击“停止”。在这种情况下，SQL 数据库和存储帐户会继续收费。如果不再需要 SQL 数据库和存储帐户，可以遵循类似过程将其删除。
...
9. 在“解决方案资源管理器”中，右键单击该解决方案（而非项目），然后选择“添加”>“新建项目”。
...
> [AZURE.NOTE] 对于 WebJobs SDK 的特定代码，请转到 [Program.cs 和 Functions.cs](#programcs) 部分。
...
类似代码获取对 *blobnamerequest* 队列的引用并创建一个新队列。在这种情况下，不需要更改权限。本教程稍后的 [ResolveBlobName](#resolveblobname) 部分将说明为何只使用 Web 应用程序写入的队列获取 Blob 名称，而不生成缩略图。

...
若要对过渡部署而非生产部署使用 SSL，你首先需要确定用于过渡部署的 URL。将云服务部署到过渡环境，而不包括证书或任何证书信息。部署后，你可以确定基于 GUID 的 URL，此 URL 将在 Azure 经典门户的“站点 URL”字段中列出。创建一个证书，使其公用名 (CN) 等于基于 GUID 的 URL（例如，**32818777-6e77-4ced-a8fc-57609d404462.chinacloudapp.cn**），然后使用 Azure 经典门户将该证书添加到过渡云服务，将证书信息添加到 CSDEF 和 CSCFG 文件，重新打包你的应用程序，然后更新过渡部署以使用新包。

<properties
	...
	description="本教程介绍了使用 Azure 媒体服务和 Azure 管理门户实施视频点播 (VoD) 内容传送应用程序的步骤。"
...
本教程介绍了使用 Azure 管理门户实施基本视频点播 (VoD) 内容传送应用程序的步骤。
...
4. 单击“保存”按钮保存更改。
	...
	分配所有新的单位大约需要 20 分钟才能完成。

	>[AZURE.NOTE] 当前，将流式处理单位的任何正值设置回“无”可将流式处理功能禁用最多 1 小时。
	>
	> 为 24 小时时间段指定的最大单位数将用于计算成本。有关定价详细信息，请参阅[媒体服务定价详细信息](/pricing/details/media-services/)。
...
请注意，除了能够使用动态打包功能以外，点播流保留单位还提供可购买的专用流出容量（以 200 Mbps 为增量来购买）。默认情况下，按需流式处理在共享实例模型中配置，该模型的服务器资源（例如计算或出口容量）与所有其他用户共享。若要增加按需流式处理吞吐量，建议购买按需流式处理保留单位。

### 编码

本部分介绍通过 Azure 管理门户使用 Media Encoder Standard 为内容编码时可以执行的步骤。
...
5. 选择其中一个“编码配置”。
	...
	[Media Encoder Standard 的任务预设字符串](https://msdn.microsoft.com/zh-cn/library/mt269960)主题说明了每个预设的含义。
...
使用 Azure 管理门户发布资产时，系统将创建定位符并提供基于 OnDemand 的 URL（如果资产包含 .ism 文件）或 SAS URL。

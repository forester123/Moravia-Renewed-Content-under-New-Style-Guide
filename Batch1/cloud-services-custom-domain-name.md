...
> [AZURE.NOTE]
某些域注册机构只允许在使用 CNAME 记录（例如 www.contoso.com）时映射子域，而不允许在使用根名称（例如 contoso.com）时映射。有关 CNAME 记录的详细信息，请参阅由的注册机构提供的文档、[关于 CNAME 记录的 Wikipedia 条目](http://zh.wikipedia.org/wiki/CNAME_record)或 [IETF 域名 - 实现和规范](http://tools.ietf.org/html/rfc1035)文档。
...
>
> 为方便起见，在暂存和生产部署之间切换或对现有部署执行就地升级时，将保留给定（生产或暂存）部署槽的 IP 地址。有关执行这些操作的详细信息，请参阅[如何管理云服务](/documentation/articles/cloud-services/cloud-services-how-to-manage/)。
...
>[AZURE.WARNING]
Azure 中的 IP 地址默认为动态 IP 地址。可能需要使用[保留 IP 地址](/documentation/articles/virtual-network/virtual-networks-reserved-public-ip/)以确保 IP 地址不变。

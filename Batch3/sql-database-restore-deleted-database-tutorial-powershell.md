...
本教程说明如何在 [Azure PowerShell](/documentation/articles/install-configure-powershell) 中还原已删除的 Azure SQL 数据库保留期内可将已删除的数据库还原到删除该数据库时的时间点。保留期由数据库的服务层决定。

还原已删除的 Azure SQL 数据库将创建新的数据库。服务会根据还原时间点使用的备份自动选择服务层。请确保逻辑服务器上有创建另一数据库所需的可用配额。如果想要请求增加配额，请联系 [Azure 支持](/support/contact/)。

...
<properties
	pageTitle="使用 Azure Resource Manager 的基本配置测试环境"
	description="了解如何使用 Resource Manager 在 Azure 中创建模拟简化 Intranet 的简单开发/测试环境。"
	documentationCenter=""
...
# 使用 Azure Resource Manager 的基本配置测试环境
...
本文分步说明如何在 Azure 虚拟网络中，使用 Resource Manager 中创建的虚拟机创建基本配置测试环境。
...
基本配置测试环境由名为 TestLab 且仅限云的虚拟网络中的公司网络子网组成，它模拟连接到 Internet 的简化专用 Intranet。
...
如果你还没有 Azure 帐户，可以在[试用 Azure](/pricing/1rmb-trial/) 中注册一个试用帐户。如果你有 MSDN 订阅，请参阅 [MSDN 订户的 Azure 权益](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/)。

> [AZURE.NOTE] Azure 中的虚拟机在运行时会持续产生货币成本。此成本是针对你的试用、MSDN 订阅或付费订阅进行计费的。有关正在运行的 Azure 虚拟机的成本的详细信息，请参阅[虚拟机定价详细信息](/home/features/virtual-machines/#price)和 [Azure 定价计算器](https://azure.microsoft.com/pricing/calculator/)。若要降低成本，请参阅[将 Azure 中测试环境虚拟机的成本降至最低](#costs)。
...
基于 Resource Manager 的虚拟机需要一个基于 Resource Manager 的存储帐户。必须为存储帐户选择只包含小写字母和数字的全局唯一名称。可以使用以下命令列出现有的存储帐户。
...
5.	当系统提示输入凭据时，请使用以下凭据：
- 名称：**DC1\**[本地管理员帐户名称]
- 密码：[本地管理员帐户密码]
6.	出现引用证书的“远程桌面连接”消息框提示时，单击“是”。

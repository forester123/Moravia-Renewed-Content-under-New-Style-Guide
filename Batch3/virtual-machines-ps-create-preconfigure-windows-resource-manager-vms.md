<properties
	...
	description="使用 PowerShell 通过 Resource Manager 部署模型创建并配置 Azure 虚拟机。"
...
# 使用 Resource Manager 和 Azure PowerShell 创建并配置 Windows 虚拟机
...
以下步骤演示如何构建一组 Azure PowerShell 命令，以创建和配置 Azure 虚拟机。可以使用此构建基块过程快速创建用于基于 Windows 的新虚拟机的命令集，并扩展现有部署。还可以使用该过程创建多个命令集，以快速构建出自定义开发/测试或 IT 专业环境。

这些步骤采用填空方法来创建 Azure PowerShell 命令集。如果你不熟悉 PowerShell 或只想知道指定什么值以成功完成配置，则此方法就会很有用。如果你是高级 PowerShell 用户，可以使用以下命令并将变量（以“$”开头的行）替换为自己的值
...
使用 Resource Manager 部署模型创建的 VM 需要一个资源组。如果需要，请为新虚拟机创建新资源组。将引号内的所有内容（包括 < and > 字符）替换为相应的名称。
...
查看可在其中创建基于 Resource Manager 的虚拟机的 Azure 位置。
...
使用 Resource Manager 部署模型创建的 VM 需要基于 Resource Manager 的存储帐户。如果需要，请使用以下命令为新虚拟机创建新存储帐户。
...
如果 Test-AzureName 命令显示“False”，则表明提供的名称是唯一的。


### 公共域名标签


使用 Resource Manager 部署模型创建的 VM 可以使用公共域名标签。该标签只能包含字母、数字和连字符。第一个和最后一个字符必须是字母或数字。

若要测试所选的域名标签是否全局唯一，请使用以下命令。
...
可以为基于 Resource Manager 的虚拟机配置入站 NAT 规则，以允许来自 Internet 的传入流量并将其放入负载平衡集。在这两种情况下，都必须指定负载均衡器实例和其他设置。有关详细信息，请参阅[使用 Azure Resource Manager 创建负载均衡器](../load-balancer/load-balancer-arm-powershell.md)。

使用 Resource Manager 部署模型创建的 VM 需要 Resource Manager 虚拟网络。如果需要，请使用新虚拟机的至少一个子网创建基于 Resource Manager 的新虚拟网络。以下示例适用于名为 **TestNet** 的新虚拟网络，其中包含名为 **frontendSubnet** 和 **backendSubnet** 的两个子网。
...
打开所选文本编辑器的新实例或 PowerShell 集成脚本环境 (ISE)，并复制以下行以启动命令集。指定此新虚拟机的资源组名称、Azure 位置和存储帐户。将引号内的所有内容（包括 < and > 字符）替换为相应的名称。

	$rgName="<resource group name>"
	$locName="<Azure location, such as West US>"
	$saName="<storage account name>"

在虚拟网络中，必须指定基于 Resource Manager 的虚拟网络的名称和子网的索引编号。使用以下命令列出虚拟网络的子网。

	$rgName="<resource group name>"
	$vnetName="<virtual network name>"
	Get-AzureRmVirtualNetwork -Name $vnetName -ResourceGroupName $rgName | Select Subnets

子网索引是此命令显示的子网编号，编号遵循从左到右的顺序并从 0 开始。
...
将以下几行内容复制到命令集，并指定现有虚拟网络的名称和虚拟机的子网索引。
...
将以下几行内容复制到命令集，并指定 NIC 的名称和全局唯一的域名标签。
...
有关如何使用入站 NAT 规则创建负载均衡器实例的信息，请参阅[使用 Azure Resource Manager 创建负载均衡器](../load-balancer/load-balancer-arm-powershell.md)。

将以下几行内容复制到命令集，并指定所需的名称和索引编号。
...
有关如何创建针对负载平衡流量创建了规则的负载均衡器实例的信息，请参阅[使用 Azure Resource Manager 创建负载均衡器](../load-balancer/load-balancer-arm-powershell.md)。
...
接下来，创建本地 VM 对象，并选择性地将该对象添加到可用性集。将以下两个选项中的一个复制到命令集，并填入名称、大小和可用性集名称。
...
若要确定选项 1 的 VM 大小字符串的可能值，请使用以下命令。
...
若要确定选项 2 的 VM 大小字符串的可能值，请使用以下命令。

	$rgName="<resource group name>"
	$avName="<availability set name>"
	Get-AzureRmVMSize -ResourceGroupName $rgName -AvailabilitySetName $avName | Select Name

> [AZURE.NOTE] 如果使用 Resource Manager，目前只能在创建虚拟机期间将其添加到可用性集。

若要将其他数据磁盘添加到 VM，请将以下几行内容复制到命令集，并指定磁盘设置。
...
接下来，需要确定虚拟机映像的发布者、产品名称和 SKU。以下是基于 Windows 的常用映像表。
...
如果未列出你需要的虚拟机映像，请按照[此处](/documentation/articles/resource-groups-vm-searching#powershell)的说明确定发布者、产品和 SKU 名称。

将以下命令复制到命令集，并填入发布者、产品和 SKU 名称。
...
最后，将以下命令复制到命令集，并填入 VM 操作系统磁盘的名称标识符。
...
复查在步骤 4 中使用文本编辑器或 PowerShell ISE 生成的 Azure PowerShell 命令集。确保已指定所有变量，并且这些变量具有正确的值。另请确保已删除所有 < and > 字符。

如果你的命令是在文本编辑器中生成的，请将命令集复制到剪贴板，然后右键单击 Azure PowerShell 提示符。这将以 PowerShell 命令序列的形式发出命令集，并创建 Azure 虚拟机。或者，通过 Azure PowerShell ISE 运行命令集。

如果要重复使用这些信息来创建其他 VM，可以将此命令集保存为 PowerShell 脚本文件 (*.ps1)。
...
[使用 Resource Manager 模板和 PowerShell 创建 Windows 虚拟机](virtual-machines-create-windows-powershell-resource-manager-template-simple)

...
<properties
	pageTitle="创建 Windows VM | Azure"
	description="使用 Azure PowerShell 和 Resource Manager 模板轻松创建新的 Windows 虚拟机。"
...
本主题介绍如何使用 Azure Resource Manager 和 PowerShell 快速创建基于 Windows 的 Azure 虚拟机。
...
最后，将上述命令集复制到剪贴板，然后右键单击打开的 Azure PowerShell 命令提示符。这将发出包含一系列 PowerShell 命令的命令集，提示你输入本地管理员帐户的名称和密码并创建 Azure 虚拟机。

...
2. 连接到媒体服务 URI
	...
	>[AZURE.NOTE] 在成功连接到 https://media.chinacloudapi.cn 之后，你将接收到指定另一个媒体服务 URI 的 301 重定向。必须对新 URI 进行后续调用。你还可能会收到包含 ODATA API 元数据说明的 HTTP/1.1 200 响应。

3. 将后续 API 调用发布到新 URL。
...
媒体服务访问控制地址为 https://wamsprodglobal001acs.accesscontrol.windows.net，中国北部地区除外，其访问控制地址为 https://wamsprodglobal001acs.accesscontrol.chinacloudapi.cn。
...
媒体服务的根 URI 为 https://media.chinacloudapi.cn/。最初应连接到此 URI，如果在响应中收到“301 重定向”，则应随后调用新 URI。此外，请勿在请求中使用任何自动重定向/跟踪逻辑。HTTP 谓词和请求正文不会转发到新 URI。

请注意，用于上传和下载资产文件的根 URI 为 https://yourstorageaccount.blob.core.chinacloudapi.cn/，其中存储帐户名为你在媒体服务帐户设置期间使用的同一帐户名。

以下示例演示了对媒体服务根 URI (https://media.chinacloudapi.cn/) 发出的 HTTP 请求。该请求将在响应中获取 301 重定向。后续请求使用新的 URI (https://wamsbayclus001rest-hs.chinacloudapi.cn/api/)。
...
>[AZURE.NOTE] 获取新 URI 后，应该使用该 URI 与媒体服务通信。

...
> [AZURE.NOTE] 本主题介绍如何从 Azure 门户创建 Docker VM。若要了解如何通过命令行创建 Docker VM，请参阅[如何从 Azure 命令行接口 (Azure CLI) 使用 Docker VM 扩展]。若要查看容器及其优点的综合讨论，请参阅 [Docker 高级白板](http://channel9.msdn.com/Blogs/Regular-IT-Guy/Docker-High-Level-Whiteboard)。
...
现在，你可以转到 [Docker 用户指南]并开始使用你的 Docker VM。如果你想要通过命令行接口在 Azure VM 上自动创建 Docker主机，请参阅[如何从 Azure 命令行接口 (Azure CLI) 使用 Docker VM 扩展]
...
<!--Link references-->
[如何从 Azure 命令行接口 (Azure CLI) 使用 Docker VM 扩展]: /documentation/articles/virtual-machines-docker-with-xplat-cli/

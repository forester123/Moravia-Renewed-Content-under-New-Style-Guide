<properties
	...
	description="使用 Resource Manager 部署模型，在 Microsoft Azure 中创建运行 Windows Server 2012 的 Oracle WebLogic Server 12c 虚拟机。"
...
3.	选择“Windows Server 2012 R2 Datacenter (en-us)”或“Windows Server 2012 R2 Datacenter (zh-cn)”映像。检查有关此映像的信息（如最小建议大小），然后单击“下一步”。
...
6. 下载 [Java JDK 1.8](http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html) 并将其安装。设置 **JAVA\_HOME** 环境变量，并将 `%JAVA_HOME%\bin` 添加到路径。
...
9.	在“指定域名和位置”对话框中，接受默认值，然后单击“下一步”。
...
12.	若要查看在 Internet 上运行的购物车应用程序，请在浏览器中打开 `http://<<unique_domain_name>>/shoppingcart` 格式的 URL。（可以通过以下方式确定 <<*unique\_domain\_name*>> 的值：在 [Azure 管理门户](https://manage.windowsazure.cn/)中单击“虚拟机”，然后选择你要用于运行 Oracle WebLogic Server 的虚拟机）。

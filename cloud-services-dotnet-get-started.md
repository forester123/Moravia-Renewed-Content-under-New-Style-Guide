...
可不订阅 Azure 而本地运行该应用，但将应用程序部署到云则需要订阅。如果你没有帐户，可以[注册试用版](/pricing/1rmb-trial)。
...
从 `Application_Start` 方法调用的代码创建*图像* Blob 容器和*图像*队列（如果它们尚不存在）。这可以确保一旦用户开始使用新的存储帐户，或在新计算机上开始使用存储仿真程序，即自动创建所需的 Blob 容器和队列。

<properties 
	...
	description="了解如何通过 Java 代码使用 Azure SQL 数据库。" 
...
Azure SQL 数据库为 Azure 提供关系数据库管理系统并且基于 SQL Server 技术。使用 SQL 数据库实例，您可以轻松地配置关系数据库解决方案并将其部署到云中，并且利用能够为企业级可用性、可缩放性和安全性提供内置数据保护和自愈优势的分布式数据中心。



## 概念
由于 Azure SQL 数据库是基于 SQL Server 技术构建的，因此通过 Java 访问 SQL 数据库与通过 Java 访问 SQL Server 非常相似。您可以本地部署应用程序（使用 SQL Server），然后通过仅更改连接字符串连接到 SQL 数据库。您可以对应用程序使用 SQL Server JDBC 驱动程序。但是，SQL 数据库和 SQL Server 之间有一些可能影响您的应用程序的差别。有关详细信息，请参阅[指导原则和限制（SQL 数据库）](http://msdn.microsoft.com/library/windowsazure/ff394102.aspx)。
...
通过 Java 代码使用 Azure SQL 数据库之前，需要创建 Azure SQL 数据库服务器。

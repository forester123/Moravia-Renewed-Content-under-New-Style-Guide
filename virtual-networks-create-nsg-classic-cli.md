...
1. 如果你从未使用过 Azure CLI，请参阅 [Install and Configure the Azure CLI](xplat-cli)（安装和配置 Azure CLI），并按照说明进行操作，直到选择 Azure 帐户和订阅。
...
3. 运行 **azure network nsg create** 命令以创建 NSG。
	...
	- **-l（或 --location）**。将在其中创建新 NSG 的 Azure 区域。对于我们的方案，为 *westus*。

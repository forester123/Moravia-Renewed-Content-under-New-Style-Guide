...
要使大数据成为可执行的分析或见解，不仅需要收集相关数据并提出正确问题，还需要确保可访问、清理和分析并以有用的方式呈现这些数据。HDInsight 中 Hadoop 上的大数据分析可用于实现这些操作。
...
* **Hadoop**（“查询”工作负荷）：为 [HDFS](#hdfs) 提供可靠的数据存储，并为并行处理和分析数据提供简单的 [MapReduce](#mapreduce) 编程模型。
...
* **<a  target="_blank" href="https://storm.incubator.apache.org/">Apache Storm</a>**（“流”工作负荷）：用于快速处理大型数据流的分布式实时计算系统。Storm 以 HDInsight 中的托管群集形式提供。请参阅[使用 Storm 和 Hadoop 分析实时传感器数据](/documentation/articles/hdinsight-storm-sensor-data-analysis/)。
...
有关开发自己的脚本操作的信息，请参阅[使用 HDInsight 开发脚本操作](/documentation/articles/hdinsight-hadoop-script-actions/)。
...
> [AZURE.NOTE] 有关特定组件的信息和版本信息，请参阅 [HDInsight 中的 Hadoop 组件、版本和服务产品][component-versioning]
...
<a target="_blank" href="http://hive.apache.org/">Apache Hive</a> 是构建于 Hadoop 上的一个数据仓库软件，允许使用类似于 SQL 的语言（称为 HiveQL）来查询和管理分布式存储中的大型数据集。Hive（类似于 Pig）是一种基于 MapReduce 的抽象。运行时，Hive 会将查询转换为一系列 MapReduce 作业。Hive 比 Pig 在概念上更接近于关系数据库管理系统，因此适用于结构化程度更高的数据。对于非结构化数据，Pig 是更佳的选择。请参阅[将 Hive 与 HDInsight 中的 Hadoop 配合使用](/documentation/articles/hdinsight-use-hive/)。

<a target="_blank" href="https://cwiki.apache.org/confluence/display/Hive/HCatalog/">Apache HCatalog</a> 是 Hadoop 的表和存储管理层，为用户提供数据的关系视图。在 HCatalog 中，可读取和写入能用于编写 Hive SerDe（序列化程序-反序列化程序）的任何格式的文件。

### <a name="mahout"></a>Mahout

<a target="_blank" href="https://mahout.apache.org/">Apache Mahout</a> 是在 Hadoop 上运行的可缩放机器学习算法库。机器学习应用程序采用统计学原理，使系统学习数据并使用以往的结果来确定将来的行为。请参阅[使用 Hadoop 上的 Mahout 生成电影推荐](/documentation/articles/hdinsight-mahout/)。
...
<a target="_blank" href="http://oozie.apache.org/">Apache Oozie</a> 是一个管理 Hadoop 作业的工作流协调系统。它与 Hadoop 堆栈集成，支持 MapReduce、Pig、Hive 和 Sqoop 的 Hadoop 作业。它也可用于安排特定于系统的作业，例如 Java 程序或 Shell 脚本。请参阅[将 Oozie 与 Hadoop 配合使用](/documentation/articles/hdinsight-use-oozie/)。
...
### <a name="yarn"></a>YARN
Apache YARN 是下一代 MapReduce（MapReduce 2.0 或 MRv2），它具有更大的可伸缩性和更强的实时处理能力，支持 MapReduce 批处理以外的数据处理方案。YARN 提供资源管理和分布式应用程序框架。MapReduce 作业将在 YARN 上运行。
...
<a  target="_blank" href="http://zookeeper.apache.org/">Apache ZooKeeper</a> 通过数据寄存器的共享层次结构命名空间 (znode) 协调大型分布式系统中的进程。Znode 包含协调流程所需的少量元数据信息：状态、位置、配置等。
...
Java 以外的许多语言也可使用 Java 虚拟机 (JVM) 运行；但可能需要在群集上安装其他组件才能运行其中某些语言。
...
* 群集缩放。通过群集缩放，无需删除或重新创建群集，即可更改运行中 HDInsight 群集的节点数。
...
使用以下资源，在此云中 Hadoop 和大数据分析介绍的基础上进行构建。
...
用户熟悉的商业智能 (BI) 工具（如 Excel、PowerPivot、SQL Server Analysis Services 和 SQL Server Reporting Services）可使用 Power Query 外接程序或 Microsoft Hive ODBC 驱动程序来检索、分析和报告与 HDInsight 集成的数据。

这些 BI 工具可有助于大数据分析：

* [使用 Power Query 将 Excel 连接到 Hadoop](/documentation/articles/hdinsight-connect-excel-power-query/)：了解如何使用 Microsoft Power Query for Excel，将 Excel 连接到存储 HDInsight 群集关联数据的 Azure 存储帐户。需要使用 Windows 工作站。可与基于 Windows 的群集共同使用。

* [使用 Microsoft Hive ODBC 驱动程序将 Excel 连接到 Hadoop](/documentation/articles/hdinsight-connect-excel-hive-ODBC-driver/)：了解如何使用 Microsoft Hive ODBC 驱动程序从 HDInsight 导入数据。需要使用 Windows 工作站。可与基于 Windows 的群集共同使用。
